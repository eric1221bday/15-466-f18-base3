...
struct Ramp
{
    ...
    Ramp(Args &&... args)
        : value(std::forward<Args>(args)...), target(value)
    {}
...
constexpr const uint32_t MixSamples =
    1024; //samples to mix at once; SDL requires a power of two; smaller values mean more reactive sound, but require more frequent audio callback invocation
