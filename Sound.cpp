...
void mix_audio(void *, Uint8 *stream, int len)
{
    ...
    for (auto si = playing_samples.begin(); si != playing_samples.end(); /* later */) {
        ...
        LR start_pan;
        compute_pan_from_listener_and_position(start_position,
                                               start_right,
...
Sample::Sample(std::string const &filename)
{
    ...
    if (!have) {
        throw std::runtime_error(
            "Failed to load WAV file '" + filename + "'; SDL says \"" + std::string(SDL_GetError()) + "\"");
    }
    ...
    if (cvt.needed) {
        std::cout << "WAV file '" + filename + "' didn't load as " + std::to_string(AudioRate)
            + " Hz, float32, mono; converting." << std::endl;
        cvt.len = audio_len;
