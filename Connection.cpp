    ...
    if (listen_socket != INVALID_SOCKET && FD_ISSET(listen_socket, &read_fds)) {
        ...
        if (got == INVALID_SOCKET) {
...
#else
            {
                ...
                std::cerr << "[" << where << "] client connected on " << connections.back().socket << "."
                          << std::endl; //INFO
    ...
    for (auto &c : connections) {
        ...
        if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            ...
            if (ret == 0) {
                ...
                std::cerr << "[" << where << "] recv() returned error " << errno << "(" << strerror(errno)
...
#else
        ssize_t ret =
            send(c.socket, reinterpret_cast< char const * >(c.send_buffer.data()), c.send_buffer.size(), MSG_DONTWAIT);
#endif
        if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            ...
            if (ret < 0) {
                ...
                std::cerr << "[" << where << "] send() returned strange number of bytes [" << ret << " of "
                          << c.send_buffer.size() << "], disconnecting." << std::endl;
            }
            c.close();
            if (on_event) on_event(&c, Connection::OnClose);
        }
...
Client::Client(std::string const &host, std::string const &port)
    : connections(1), connection(connections.front())
{
#ifdef _WIN32
    { //init winsock:
        WSADATA info;
